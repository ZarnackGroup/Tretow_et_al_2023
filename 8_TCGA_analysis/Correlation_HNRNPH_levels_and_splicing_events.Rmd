...
HNRNPH1nmdIsoformPSIs <- readRDS(paste0(tcgaDir,"/rds_files/BRCA_PSI.rds")) %>% dplyr::filter(eventType == "HNRNPH1nmdIsoform")

plotdf <- left_join(HNRNPHexpression, HNRNPH1nmdIsoformPSIs, by="tcgaSampleBarcode")
ggplot(plotdf %>% dplyr::filter(sampleType.x == "Primary Tumor"), aes(x=HNRNPH1, y=PSI)) +
    rasterise(geom_pointdensity(size=3), dpi=300) +
    geom_smooth(method="lm", se=FALSE) +
    scale_color_viridis() +
    stat_cor() +
    labs(x="HNRNPH1 expression (vst)", y="HNRNPH1 NMD PSI") +
    myTheme +
    theme(aspect.ratio=1)
#ggsave(filename = "~/Desktop/H/final/2.pdf")

ggplot(plotdf %>% dplyr::filter(sampleType.x == "Primary Tumor"), aes(x=HNRNPH2, y=PSI)) +
    rasterise(geom_pointdensity(size=3), dpi=300) +
    geom_smooth(method="lm", se=FALSE) +
    scale_color_viridis() +
    stat_cor() +
    labs(x="HNRNPH2 expression (vst)", y="HNRNPH1 NMD PSI") +
    myTheme +
    theme(aspect.ratio=1)
...
# New JONAS
```{r}
test_res <- rbind((aov(r ~ hillCat, data = corRes %>% filter(gene=="HNRNPH1")) %>% rstatix::tukey_hsd()) %>% cbind(gene="HNRNPH1"), 
                  (aov(r ~ hillCat, data = corRes %>% filter(gene=="HNRNPH2")) %>% rstatix::tukey_hsd()) %>% cbind(gene="HNRNPH2")) #%>% rstatix::add_x_position(x="gene", dodge=0.8)
test_res$y.position <- seq(0.65, 1000, by=0.1) %>% head(nrow(test_res)/2) %>% rep(2)
test_res$xmin <- c(0.73, 0.73, 1, 1.73, 1.73, 2)
test_res$xmax <- c(1, 1.27, 1.27, 2, 2.27, 2.27)

ggplot(data=corRes, mapping=aes(x=gene, y=r)) +
        geom_quasirandom(data=corRes, aes(col=hillCat, fill=hillCat), dodge.width = .8, size=1.25) +
        geom_boxplot(data=corRes, aes(col=hillCat, fill=hillCat), alpha=.5, outlier.size = -1, col="black", position = position_dodge(width = .8), width = .7) +
        ggpubr::stat_pvalue_manual(test_res, tip.length=0) +
        scale_fill_manual(values=c("Enhanced"="cornflowerblue", "Repressed"="salmon2", "Ctrl"="#606060")) +
        scale_color_manual(values=c("Enhanced"="cornflowerblue", "Repressed"="salmon2", "Ctrl"="#606060")) +
        coord_cartesian(ylim=c(-0.6, 0.9)) +
        scale_y_continuous(breaks=seq(-1, 1, by=0.5)) +
        labs(x="", y="Correlation coefficnt") +
        myTheme + theme(legend.position = "right", aspect.ratio=1.5)
```
