...
variants <- read_csv("/Users/jb/data/HNPRNPH/additional analysis/G4mer/rG4_disrupting_hnrnp_regions_breast_cancer_associated_pmbb_variants.csv")

# Reshape wt_score and mutated_score into one column
plot_df <- variants %>% select(snp, wt_score, mutated_score) %>% distinct() %>% 
    pivot_longer(cols = c(wt_score, mutated_score), names_to = "type", values_to = "score") %>%
    mutate(type = factor(type, levels = c("wt_score", "mutated_score")))

# Make the grouped bar plot
ggplot(plot_df, aes(x = snp, y = score, fill = type)) +
    geom_col(position = "dodge") +
    geom_hline(yintercept=0.5, lty=2)+
    labs(x = "SNP", y = "Score", fill = "Type") +
    scale_fill_manual(values = c("wt_score" = "darkgrey", "mutated_score" = "goldenrod3"),
                      labels = c("wt_score" = "WT", "mutated_score" = "MT")) +
    theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), aspect.ratio = 1)
ggsave("/Users/jb/data/HNPRNPH/additional analysis/G4mer/a.pdf")
